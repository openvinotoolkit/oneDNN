...
struct post_ops : public handle<dnnl_post_ops_t> {
    ...
    void append_dw_conv(int in_h, int in_w, int ker_h, int ker_w, int str_h,
            int str_w, dnnl_data_type_t in_dt, const float *weights_data,
            const float *biases_data) {
        error::wrap_c_api(
                dnnl_post_ops_append_dw_conv(get(), in_h, in_w, ker_h, ker_w,
                        str_h, str_w, in_dt, weights_data, biases_data),
    ...
    void append_quantization(algorithm alg, const void *crop_low,
            const void *crop_high, const void *input_scale,
            const void *input_shift, const void *output_scale,
            const void *output_shift) {
        error::wrap_c_api(
                dnnl_post_ops_append_quantization(get(), convert_to_c(alg),
                        crop_low, crop_high, input_scale, input_shift,
...
struct primitive_attr : public handle<dnnl_primitive_attr_t> {
    ...
    void get_output_compensations(
            int &mask, std::vector<int32_t> &compensations) const {
        int count, c_mask;
        const int32_t *c_compensations;
        error::wrap_c_api(dnnl_primitive_attr_get_output_compensations(
                                  get(), &count, &c_mask, &c_compensations),
    ...
    void set_output_compensations(
            int mask, const std::vector<int32_t> &compensations) {
        error::wrap_c_api(
                dnnl_primitive_attr_set_output_compensations(get(),
                        (int)compensations.size(), mask, &compensations[0]),
                "could not set int output compensations");
    }

    void get_input_zero_points(
            int &mask, std::vector<uint8_t> &zero_points) const {
        int count, c_mask;
        const uint8_t *c_zero_points;
        error::wrap_c_api(dnnl_primitive_attr_get_input_zero_points(
                                  get(), &count, &c_mask, &c_zero_points),
    ...
    void set_input_zero_points(
            int mask, const std::vector<uint8_t> &zero_points) {
        error::wrap_c_api(
                dnnl_primitive_attr_set_input_zero_points(
                        get(), (int)zero_points.size(), mask, &zero_points[0]),
                "could not set int input zero_points");
    }

    void get_weights_zero_points(
            int &mask, std::vector<int8_t> &zero_points) const {
        int count, c_mask;
        const float *c_zero_points;
        error::wrap_c_api(dnnl_primitive_attr_get_weights_zero_points(
                                  get(), &count, &c_mask, &c_zero_points),
    ...
    void set_weights_zero_points(
            int mask, const std::vector<float> &zero_points) {
        error::wrap_c_api(
