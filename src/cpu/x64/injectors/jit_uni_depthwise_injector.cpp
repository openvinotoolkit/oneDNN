...
void jit_uni_depthwise_injector_f32<isa>::compute_body(size_t start_idx,
        ...
        const Xbyak::Reg64 &p_bias, bool is_broadcast) {
    for (size_t idx = start_idx; idx < end_idx; idx++) {
        switch (depthwise_alg) {
            case alg_kind::depthwise_scale_shift:
                scale_shift_compute_vector(
                        Vmm(idx), p_weights, p_bias, is_broadcast);
                break;
...
void jit_uni_depthwise_injector_f32<isa>::init_ptrs(
    ...
    h->mov(reg_d_weights,
            reinterpret_cast<size_t>(post_op_.depthwise.weights_data));
    if (post_op_.depthwise.alg == alg_kind::depthwise_scale_shift)
        h->mov(reg_d_bias,
                reinterpret_cast<size_t>(post_op_.depthwise.biases_data));
