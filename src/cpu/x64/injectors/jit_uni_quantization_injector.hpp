...
struct dynamic_params_t {
    dynamic_params_t() :
            reg_oc_off(Xbyak::Reg64(0)), reg_oc_off_addr(), vmm_idx_off({}), dst_dt(dnnl_f32), useAddr(false) {
    }
    ...
    dynamic_params_t(Xbyak::Reg64 reg_oc_off, const std::map<size_t, int>& vmm_idx_off, data_type_t dst_dt,
            ...
            reg_oc_off(reg_oc_off), reg_oc_off_addr(), vmm_idx_off(vmm_idx_off), dst_dt(dst_dt), useAddr(false),
