...
void jit_uni_quantization_injector_f32<isa, Vmm>::init_crop_ptrs(
        const Xbyak::Operand &ch_off) {
    h->mov(reg_d_weights_,
            reinterpret_cast<size_t>(
                    post_op_.quantization.crop_low_data->shifts_));
    h->mov(reg_d_bias_,
            reinterpret_cast<size_t>(
                    post_op_.quantization.crop_high_data->shifts_));

    if (post_op_.quantization.crop_low_data->count_ != 1
            && !post_op_.quantization.crop_low_data->has_default_values())
        h->add(reg_d_weights_, ch_off);
    if (post_op_.quantization.crop_high_data->count_ != 1
            && !post_op_.quantization.crop_high_data->has_default_values())
...
void jit_uni_quantization_injector_f32<isa, Vmm>::compute_crop(int start_idx,
    ...
    if (is_scalar) {
        ...
        if (post_op_.quantization.crop_low_data->count_ == 1)
            ...
            h->uni_vbroadcastss(
                    vmm_d_weights_, h->ptr[reg_d_weights_ + offset]);
        else
...
void jit_uni_quantization_injector_f32<isa, Vmm>::init_input_scale_shift_ptrs(
    ...
    h->mov(reg_d_weights_,
            reinterpret_cast<size_t>(
                    post_op_.quantization.input_scale_data->scales_));
    h->mov(reg_d_bias_,
            reinterpret_cast<size_t>(
                    post_op_.quantization.input_shift_data->shifts_));

    if (post_op_.quantization.input_scale_data->count_ != 1)
        h->add(reg_d_weights_, ch_off);
    if (post_op_.quantization.input_shift_data->count_ != 1
            && !post_op_.quantization.input_shift_data->has_default_values())
        h->add(reg_d_bias_, ch_off);
}
...
void jit_uni_quantization_injector_f32<isa, Vmm>::init_output_scale_shift_ptrs(
    ...
    h->mov(reg_d_weights_,
            reinterpret_cast<size_t>(
                    post_op_.quantization.output_scale_data->scales_));
    h->mov(reg_d_bias_,
            reinterpret_cast<size_t>(
                    post_op_.quantization.output_shift_data->shifts_));

    if (post_op_.quantization.output_scale_data->count_ != 1)
        h->add(reg_d_weights_, ch_off);
    if (post_op_.quantization.output_shift_data->count_ != 1
            && !post_op_.quantization.output_shift_data->has_default_values())
        h->add(reg_d_bias_, ch_off);
}
