        ...
        if (gemm_batch > 0 && brg_kernel != nullptr) {
            ...
            for (int b = 0; b < gemm_batch; b++) {
                ...
                if (jbgp.weights_compressed) {
                    ...
                    int typesize_scale = jbgp.orig_wei_dt == data_type::u2 ? 4 :
                        (one_of(jbgp.orig_wei_dt, data_type::nf4, data_type::s4, data_type::u4, data_type::f4_e2m1) ? 2 : 1);
                    ...
                    const int ic_internal_block = pd()->jbgp_.orig_wei_dt == data_type::u2 ? 4 : pd()->jbgp_.wei_dt == data_type::bf16 ||
