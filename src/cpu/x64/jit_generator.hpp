...
public:
    ...
    static bool imm_fits_int32(dim_t imm) {
    ...
    void cmp(const Xbyak::Operand &op, T imm) {
        JIT_ASSERT(imm_fits_int32(imm));
        Xbyak::CodeGenerator::cmp(op, static_cast<uint32_t>(imm));
    }

    // Xbyak's `imul` takes a signed `int` immediate.
    template <typename T, enable_if_wide_imm_t<T> = 0>
    void imul(const Xbyak::Reg32e &dst, const Xbyak::Operand &src, T imm) {
        JIT_ASSERT(imm_fits_int32(imm));
