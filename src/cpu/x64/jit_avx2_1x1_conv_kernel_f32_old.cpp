            ...
            if (post_op.is_eltwise()) {
                ...
                mov(reg_d_weights,
                        reinterpret_cast<size_t>(
                                post_op.depthwise.weights_data));
                mov(reg_d_bias,
                        reinterpret_cast<size_t>(
                                post_op.depthwise.biases_data));
    ...
    for (int i = 0; i < end_idx; i++) {
        ...
        if (post_op.is_eltwise()) {
            ...
            depthwise_injectors.push_back(
                    new jit_uni_depthwise_injector_f32<avx2>(
...
status_t jit_avx2_1x1_conv_kernel_f32_old::init_conf(jit_1x1_conv_conf_t &jcp,
    ...
    if (one_of(jcp.prop_kind, forward_training, forward_inference)) {
        ...
        load_blocking = jcp.with_dw_conv
                ? static_cast<int>(nstl::min<dim_t>(3 * jcp.load_block, jcp.oc))
