...
#define GET_OFF_DW(field) offsetof(jit_conv_args_t, field)
...
void jit_uni_dw_conv_row_f32<isa>::apply_filter(int ur_w, int kw_size) {
    ...
    for (int i = 0; i < repeats; i++) {
        for (int kw = 0; kw < kw_size; kw++) {
            ...
            for (int ow = 0; ow < ur_w; ow++) {
                int inp_off = ow * stride_w * ch_blk + kw * ch_blk
                        + i * (jcp.ch_block / 2);

                Vmm vmm_src = get_src_reg(0);
                load_src(vmm_src,
                        ptr[aux_reg_input0 + inp_off * jcp.typesize_in]);
    ...
    for (int i = 0; i < repeats; i++) {
        for (int kw = 0; kw < kw_size; kw++) {
            ...
            for (int ow = 0; ow < ur_w; ow++) {
                ...
                Vmm vmm_src = get_src_reg(0);
                load_src(vmm_src,
                        ptr[aux_reg_input1 + inp_off * jcp.typesize_in]);
    ...
    for (int i = 0; i < repeats; i++) {
        for (int kw = 0; kw < kw_size; kw++) {
            ...
            for (int ow = 0; ow < ur_w; ow++) {
                ...
                Vmm vmm_src = get_src_reg(0);
                load_src(vmm_src,
                        ptr[aux_reg_input2 + inp_off * jcp.typesize_in]);
...
template <cpu_isa_t isa>
void jit_uni_dw_conv_row_f32<isa>::cvt2ps(
        data_type_t type_in, Vmm vmm_in, const Operand &op, bool scalar_load) {
...
void jit_uni_dw_conv_row_f32<isa>::apply_postprocessing(int ur_w, int oc_step) {
    ...
    for (int r = 0; r < repeats; r++) {
        for (int ow = 0; ow < ur_w; ow++) {
            if (jcp.src_dt == data_type::u8) {
                uni_vcvtdq2ps(
                        get_acc_reg(r * ur_w + ow), get_acc_reg(r * ur_w + ow));
            }

            if (jcp.with_bias) {
                int b_off = r * (jcp.ch_block / 2);
                cvt2ps(jcp.bia_dt, vmm_bias,
                        ptr[reg_bias + b_off * jcp.typesize_bia], false);
                uni_vaddps(get_acc_reg(r * ur_w + ow),
                        get_acc_reg(r * ur_w + ow), vmm_bias);
            }
        }
    }
    ...
    if (jcp.with_sum) {
        ...
        for (int i = start_idx; i < p.len(); i++) {
            ...
            if (post_op.is_sum()) { sum_dt = post_op.sum.dt; }
        }

        for (int r = 0; r < repeats; r++) {
            int tail_size = isa == sse41
                    ? nstl::min(
                              jcp.ch_block / 2, oc_step - r * jcp.ch_block / 2)
                    : oc_step;
            bool is_scalar_store = isa == sse41 ? tail_size < jcp.ch_block / 2
                                                : tail_size < jcp.ch_block;
            ...
            for (int ow = 0; ow < ur_w; ow++) {
                if (is_scalar_store) {
                    if (isa == avx512_core) {
                        ...
                        cvt2ps(sum_dt, vmm_in,
                                ptr[reg_output + o_off * jcp.typesize_out],
                                false);
                        uni_vaddps(get_acc_reg(r * ur_w + ow),
                                get_acc_reg(r * ur_w + ow), vmm_sum);
                    } else {
                        for (int oc = 0; oc < tail_size; oc++) {
                            ...
                            cvt2ps(sum_dt, vmm_sum,
                                    ptr[reg_output + o_off * jcp.typesize_out],
                                    true);

                            if (oc >= jcp.ch_block / 2) {
                                vperm2i128(Ymm(vmm_sum.getIdx()),
                                        Ymm(vmm_sum.getIdx()),
                                        Ymm(vmm_sum.getIdx()), 0x01);
                            }
                            uni_vpslldq(vmm_sum, vmm_sum,
                                    jcp.typesize_out
                                            * (oc % (jcp.ch_block / 2)));
                    ...
                    uni_vpxor(vmm_sum, vmm_sum, vmm_sum);
                    cvt2ps(sum_dt, vmm_sum,
                            ptr[reg_output + o_off * jcp.typesize_out], false);
    ...
    for (int i = start_idx; i < p.len(); i++) {
        ...
        if (post_op.is_eltwise()) {
            ...
            depthwise_injectors[depthwise_inj_idx]->compute_vector_range(
                    4, 4 + ur_w, reg_d_weights, reg_d_weights);

            if (repeats == 2) {
                add(reg_d_weights, (jcp.ch_block / 2) * sizeof(float));

                depthwise_injectors[depthwise_inj_idx]->compute_vector_range(
                        4 + ur_w, 4 + 2 * ur_w, reg_d_weights, reg_d_weights);
            }
            ...
            bool do_rounding = do_dequantization || jcp.dst_dt == dnnl_f32
                    || i != p.len() - 1;

            const Xbyak::RegExp quant_arg_base
                    = this->rsp + post_ops_data_offset;
            quantization_injectors[quantization_inj_idx]->init_crop_ptrs(
                    quant_arg_base, reg_oc_off);
            for (int r = 0; r < repeats; r++) {
                int s_idx = get_acc_reg(r * ur_w).getIdx();
                quantization_injectors[quantization_inj_idx]->compute_crop(
                        s_idx, s_idx + ur_w,
                        r * (jcp.ch_block / 2) * sizeof(float));
            }

            quantization_injectors[quantization_inj_idx]
                    ->init_input_scale_shift_ptrs(quant_arg_base, reg_oc_off);
            for (int r = 0; r < repeats; r++) {
                int s_idx = get_acc_reg(r * ur_w).getIdx();
                quantization_injectors[quantization_inj_idx]
                        ->compute_input_scale_shift(s_idx, s_idx + ur_w,
                                r * (jcp.ch_block / 2) * sizeof(float),
                                do_rounding);
            }

            quantization_injectors[quantization_inj_idx]
                    ->init_output_scale_shift_ptrs(quant_arg_base, reg_oc_off);
            for (int r = 0; r < repeats; r++) {
                int s_idx = get_acc_reg(r * ur_w).getIdx();
                quantization_injectors[quantization_inj_idx]
                        ->compute_output_scale_shift(s_idx, s_idx + ur_w,
                                r * (jcp.ch_block / 2) * sizeof(float));
            }
...
template <cpu_isa_t isa>
void jit_uni_dw_conv_row_f32<isa>::store_dst_typed(
    ...
    switch (jcp.dst_dt) {
        ...
        case data_type::s8:
            ...
            if (isa != sse41 && !scalar_store) vpermq(ymm_dst, ymm_dst, 0x08);
...
void jit_uni_dw_conv_row_f32<isa>::store_dst(int ur_w, int oc_step) {
    ...
    for (int i = 0; i < repeats; i++) {
        int tail_size = isa == sse41
                ? nstl::min(jcp.ch_block / 2, oc_step - i * jcp.ch_block / 2)
                : oc_step;
        bool is_scalar_store = isa == sse41 ? tail_size < jcp.ch_block / 2
                                            : tail_size < jcp.ch_block;
        if (is_scalar_store) {
            for (int ow = 0; ow < ur_w; ow++) {
                ...
                if (isa == avx512_core) {
                    ...
                    store_dst_typed(ptr[reg_output + o_off * jcp.typesize_out],
                            vmm_dst | ktail_mask, false);
                } else {
                    for (int oc = 0; oc < tail_size; oc++) {
                        int o_off
                                = ow * ow_stride_ + i * (jcp.ch_block / 2) + oc;
                        store_dst_typed(
                                ptr[reg_output + o_off * jcp.typesize_out],
                                vmm_dst, true);
                        ...
                        if (isa == sse41) {
                            ...
                            vpalignr(ymm_dst, vmm_tmp, ymm_dst,
                                    jcp.typesize_out);
                        }
                    }
                }
...
void jit_uni_dw_conv_row_f32<isa>::loop_body(int oc_step) {
    ...
    L(left_pad_label);
    {
        ...
        add(reg_input0,
                jcp.typesize_in * ur_w * jcp.ch_block * (jcp.stride_w - 1));
        add(reg_input1,
                jcp.typesize_in * ur_w * jcp.ch_block * (jcp.stride_w - 1));
        add(reg_input2,
                jcp.typesize_in * ur_w * jcp.ch_block * (jcp.stride_w - 1));
...
void jit_uni_dw_conv_row_f32<isa>::generate() {
    ...
    for (int i = start_idx; i < p.len(); i++) {
        ...
        if (post_op.is_eltwise()) {
            ...
            quantization_injectors.push_back(
                    new jit_uni_quantization_injector_f32<isa>(this, post_op,
                            vmm_d_weights, vmm_d_bias, reg_d_weights,
                            reg_d_bias));
        }
    }
    ...
    if (post_ops_pointers_count != 0) {
        ...
        mov(aux_reg0,
                ptr[this->param1 + GET_OFF_DW(post_ops_binary_rhs_arg_vec)]);
    ...
    if (jcp.with_bias) mov(reg_bias, ptr[this->param1 + GET_OFF_DW(bias)]);
    ...
    if (jcp.oc % jcp.ch_block != 0) loop_body(jcp.oc % jcp.ch_block);
...
template <cpu_isa_t isa>
bool jit_uni_dw_conv_row_f32<isa>::post_ops_ok(
    ...
    auto contain = [&](dnnl::impl::primitive_kind_t kind) {
        return p.find(kind, start_idx, -1) != -1;
    };
    auto position = [&](dnnl::impl::primitive_kind_t kind) {
        return p.find(kind, start_idx, -1);
    };
    auto count = [&](dnnl::impl::primitive_kind_t kind) {
        return p.count(kind, start_idx, -1);
    };
    ...
    return all_post_ops_supported() && count(primitive_kind::sum) <= 1
            ...
            && IMPLICATION(contain(primitive_kind::sum),
                    position(primitive_kind::sum) == start_idx)
            && IMPLICATION(contain(primitive_kind::binarization),
                    position(primitive_kind::binarization) == p.len() - 1)
            && IMPLICATION(contain(primitive_kind::binarization),
                    !contain(primitive_kind::sum));
}

template <cpu_isa_t isa>
status_t jit_uni_dw_conv_row_f32<isa>::init_conf(jit_1x1_conv_conf_t &jcp,
        jit_conv_conf_t &jcp_dw, const primitive_attr_t &attr) {
    ...
    if (jcp_dw.kh != 3 || jcp_dw.kw != 3) return status::unimplemented;

    if (!post_ops_ok(jcp_dw, attr)) return status::unimplemented;
