...
status_t jit_uni_x8s8s32x_1x1_convolution_fwd_t<isa>::execute_forward(
    ...
    if (pd()->jcp_.with_dw_conv) {
        ...
        int dw_wei_mask = attr_dw->scales_.get_mask(DNNL_ARG_WEIGHTS);
        float dw_factor = 1.f / jcp_dw->wei_adj_scale;
        if (dw_wei_mask == 0) {
            utils::array_set(dw_local_scales,
                    dw_wei_scales[0] / dst_scales[0] * dw_factor,
                    pd()->jcp_.ic_block);
        } else {
            for (dim_t c = 0; c < pd()->dw_conv_pd_->OC(); c++)
                dw_local_scales[c] = dw_wei_scales[c] / dst_scales[0] * dw_factor;
        }
