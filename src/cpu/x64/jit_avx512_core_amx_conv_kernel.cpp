...
void jit_avx512_core_amx_fwd_kernel_t::generate() {
    ...
    if (postops_injector_)
        postops_injector_->push_post_ops_data_on_stack(param1,
                GET_OFF(post_ops_binary_rhs_arg_vec), reg_inp_ptr, reg_wei_ptr);
...
void jit_avx512_core_amx_bwd_data_kernel_t::store_output_vector_xf16(
    ...
    const int eltwise_ind = p.find(primitive_kind::eltwise);
    if (eltwise_ind != -1)
        idx_to_eltwise_injector_.at(eltwise_ind)
                .compute_vector(zmm_out.getIdx());
...
void jit_avx512_core_amx_bwd_data_kernel_t::store_output_vector_int8(
    ...
    /* Do post-ops */
    if (maybe_eltwise(0))
        idx_to_eltwise_injector_.at(0).compute_vector(zmm_out.getIdx());
    ...
    if (maybe_eltwise(1))
        idx_to_eltwise_injector_.at(1).compute_vector(zmm_out.getIdx());
...
void jit_avx512_core_amx_bwd_weights_kernel_t::compute_full_spat_loop(
    ...
    auto emit_kh_kw_loop = [&](bool is_first_block, bool is_last_block) {
        ...
        L(kh_loop);
        {
            ...
            mul_by_const(reg_ihs, reg_tmp, static_cast<int>(get_src_offset(0, 0, 1)));
            mul_by_const(reg_ohs, reg_tmp, static_cast<int>(get_ddst_offset(0, 1)));
