...
status_t jit_avx512_core_x8s8s32x_deconv_fwd_kernel_vmm_t::init_conf(
    ...
    if (jcp.with_depthwise || jcp.with_quantization) { max_regs -= 2; }
...
bool jit_avx512_core_x8s8s32x_deconv_fwd_kernel_vmm_t::post_ops_ok(
    ...
    return injector::post_ops_ok(post_ops_ok_args_t(avx512_core,
            {eltwise, binary, sum, depthwise, quantization}, post_ops, &dst_d,
    ...
    for (int ocb = 0; ocb < jcp.nb_oc_blocking; ocb++) {
        ...
        for (int ur = 0; ur < ur_w; ur++) {
            ...
            if (jcp.wei_adj_scale != 1.f) {
                ...
                vmulps(mask_vmm, vmm, vmm_scale_adjust);
            }
