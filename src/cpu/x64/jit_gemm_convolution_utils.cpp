        ...
        for (int i = 0; i < post_ops_.len(); i++) {
            ...
            if (post_op.is_eltwise()) {
                ...
                jit_depthwise_injectors_.push_back(
                        new jit_uni_depthwise_injector_f32<isa>(this,
                                post_op.depthwise.alg, depthwise_reserved_2_));
            } else {
                only_eltwise = false;
            }
        }
        ...
        for (int i = 0; i < post_ops_.len(); i++) {
            ...
            if (post_op.is_eltwise()) {
                ...
                mov(reg_d_bias,
                        reinterpret_cast<size_t>(
                                post_op.quantization
                                        .data[post_op.quantization.crop_high]));
                if (post_op.quantization
                ...
                mov(reg_d_weights,
                        reinterpret_cast<size_t>(
                                post_op.quantization
                                        .data[post_op.quantization.inp_scale]));
                if (post_op.quantization
                ...
                mov(reg_d_bias,
                        reinterpret_cast<size_t>(
                                post_op.quantization
                                        .data[post_op.quantization.inp_shift]));
                if (post_op.quantization
