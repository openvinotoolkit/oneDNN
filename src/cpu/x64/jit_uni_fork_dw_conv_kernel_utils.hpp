...
status_t jit_uni_fork_dw_conv_fwd_kernel<isa, kernel_dt>::init_conf(
    ...
    jcp.kd = (ndims == 5) ? static_cast<int>(weights_d.dims()[3]) : 1;
    jcp.kh = (ndims == 3) ? 1 : static_cast<int>(weights_d.dims()[ndims - 1]);
    ...
    jcp.back_pad = (ndims == 5) ? static_cast<int>(cd.padding[1][0]) : 0;
    jcp.b_pad = (ndims == 3) ? 0 : static_cast<int>(cd.padding[1][ndims - 4]);
    jcp.r_pad = static_cast<int>(cd.padding[1][ndims - 3]);
...
status_t jit_uni_fork_dw_conv_bwd_data_kernel<isa, kernel_dt>::init_conf(
    ...
    jcp.b_pad = static_cast<int>(cd.padding[1][0]);
    jcp.r_pad = static_cast<int>(cd.padding[1][1]);
