...
#ifndef CPU_REORDER_SIMPLE_SPARSE_REORDER_V0_HPP
#define CPU_REORDER_SIMPLE_SPARSE_REORDER_V0_HPP
...
template <SIMPLE_SPARSE_REORDER_V0_TEMPL_DECL, typename spec = void>
struct simple_sparse_reorder_v0_impl {};

namespace {
template <typename T>
constexpr bool is_format_tag_v(T) {
...
struct simple_sparse_reorder_v0_impl<SIMPLE_SPARSE_REORDER_V0_TEMPL_CALL,
    ...
    static status_t is_applicable(const memory_desc_wrapper &input_d,
        ...
        //         VERBOSE_UNSUPPORTED_TENSOR_LAYOUT, "dst");
        VDISPATCH_REORDER_IC(
            utils::one_of(input_d.data_type(), data_type::f32, data_type::s8),
    ...
    static status_t execute(const cpu_reorder_pd_t *pd, const exec_ctx_t &ctx) {
        ...
        const int OC = static_cast<int>(input_dims[0]);
        const int NB_OC = static_cast<int>(padded_dims[0] / o_blksize);
        const int IC = static_cast<int>(input_dims[1]);
        const int NB_IC = static_cast<int>(padded_dims[1] / i_blksize);
        const int plain_o_stride = static_cast<int>(input_d.blocking_desc().strides[0]);
        const int plain_i_stride = static_cast<int>(input_d.blocking_desc().strides[1]);
        ...
        auto output_offset = static_cast<int>(ceil((float)total_blocks * sizeof(comp_tile_len_type) / 64.0) * 64);
                ...
                for (int ic_base = 0; ic_base < ic_block;
                    ...
                    for (int oc = 0; oc < oc_block; oc++) { // 64
                        ...
                        for (int ic = 0; ic < ic_block_here; ic++) { // 4
                            data_t<type_o> o = static_cast<data_t<type_o>>(inp[plain_off]);
                            if (o != 0) {
                                *outp++ = o;
                                bitmask_ptr[bitmask_idx] |= (uint64_t {1} << bit);
                                non_zeros++;
                            }
...
template <SIMPLE_SPARSE_REORDER_V0_TEMPL_DECL, typename spec = void>
struct simple_sparse_reorder_v0_t : public primitive_t {
    struct pd_t : public cpu_reorder_pd_t {
        using cpu_reorder_pd_t::cpu_reorder_pd_t;

        DECLARE_COMMON_PD_T("simple_sparse_v0:any", simple_sparse_reorder_v0_t);
...
#undef SIMPLE_SPARSE_REORDER_V0_TEMPL_DECL
#undef SIMPLE_SPARSE_REORDER_V0_TEMPL_CALL
