...
struct dnnl_post_ops : public dnnl::impl::c_compatible {
    struct entry_t {
        ...
        struct quantization_t {
            ...
            const dnnl::impl::rnn_create_time_scales_t *input_scale_data;
            const dnnl::impl::shifts_t<float> *input_shift_data;
            const dnnl::impl::rnn_create_time_scales_t *output_scale_data;
            ...
            switch (kind) {
                ...
                case primitive_kind::depthwise:
                    ret = depthwise.alg == rhs.depthwise.alg
                            && depthwise.weights_data
                                    == rhs.depthwise.weights_data
                            && depthwise.biases_data
                                    == rhs.depthwise.biases_data;
                    break;
                case primitive_kind::quantization:
                    ret = quantization.alg == rhs.quantization.alg
                            && quantization.crop_low_data
                                    == rhs.quantization.crop_low_data
                            && quantization.crop_high_data
                                    == rhs.quantization.crop_high_data
                            && quantization.input_scale_data
                                    == rhs.quantization.input_scale_data
                            && quantization.input_shift_data
                                    == rhs.quantization.input_shift_data
                            && quantization.output_scale_data
                                    == rhs.quantization.output_scale_data
                            && quantization.output_shift_data
                                    == rhs.quantization.output_shift_data;
                    break;
...
struct dnnl_primitive_attr : public dnnl::impl::c_compatible {
    ...
    enum class skip_mask_t : unsigned {
        ...
        input_zero_points = 1u << 19,
        weights_zero_points = 1u << 20,
        output_compensations = 1u << 21,
    };
