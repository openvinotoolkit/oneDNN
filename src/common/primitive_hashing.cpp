    ...
    for (int i = 0; i < attr.post_ops_.len(); i++) {
        ...
        switch (entry.kind) {
            ...
            case primitive_kind::depthwise:
                ...
                seed = hash_combine(seed,
                        reinterpret_cast<size_t>(entry.depthwise.weights_data));
                seed = hash_combine(seed,
                        reinterpret_cast<size_t>(entry.depthwise.biases_data));
                break;
            case primitive_kind::quantization:
                ...
                seed = hash_combine(seed,
                        reinterpret_cast<size_t>(
                                entry.quantization.crop_high_data));
                seed = hash_combine(seed,
                        reinterpret_cast<size_t>(
                                entry.quantization.crop_low_data));
                seed = hash_combine(seed,
                        reinterpret_cast<size_t>(
                                entry.quantization.input_scale_data));
                seed = hash_combine(seed,
                        reinterpret_cast<size_t>(
                                entry.quantization.input_shift_data));
                seed = hash_combine(seed,
                        reinterpret_cast<size_t>(
                                entry.quantization.output_scale_data));
                seed = hash_combine(seed,
                        reinterpret_cast<size_t>(
                                entry.quantization.output_shift_data));
                break;
