...
status_t post_ops_t::append_depthwise(
        alg_kind_t alg, const float *weights_data, const float *biases_data) {
...
status_t post_ops_t::append_quantization(alg_kind_t alg, const void *crop_low,
        const void *crop_high, const void *input_scale, const void *input_shift,
...
status_t dnnl_primitive_attr_get_output_compensations(
        const primitive_attr_t *attr, int *count, int *mask,
        const int32_t **compensations) {
    if (any_null(attr, count, mask, compensations)) return invalid_arguments;
...
status_t dnnl_primitive_attr_get_input_zero_points(const primitive_attr_t *attr,
    ...
    if (any_null(attr, count, mask, zero_points)) return invalid_arguments;
...
status_t dnnl_primitive_attr_get_weights_zero_points(
        const primitive_attr_t *attr, int *count, int *mask,
...
status_t dnnl_primitive_attr_set_weights_zero_points(
        primitive_attr_t *attr, int count, int mask, const float *zero_points) {
    bool ok = !any_null(attr, zero_points) && count > 0 && mask >= 0;
    if (!ok) return invalid_arguments;
...
status_t dnnl_post_ops_append_depthwise(dnnl_post_ops_t post_ops,
        dnnl_alg_kind_t alg, const float *weights_data,
...
status_t dnnl_post_ops_append_quantization(post_ops_t *post_ops,
        alg_kind_t kind, const void *crop_low, const void *crop_high,
        const void *input_scale, const void *input_shift,
        const void *output_scale, const void *output_shift) {
    if (post_ops == nullptr) return invalid_arguments;

    return post_ops->append_quantization(kind, crop_low, crop_high, input_scale,
            input_shift, output_scale, output_shift);
}
...
status_t dnnl_post_ops_append_dw_conv(post_ops_t *post_ops, int in_h, int in_w,
        ...
        dnnl::impl::data_type_t in_dt, const float *weights_data,
        const float *biases_data) {
    if (post_ops == nullptr) return invalid_arguments;

    return post_ops->append_dw_conv(in_h, in_w, ker_h, ker_w, str_h, str_w,
            in_dt, weights_data, biases_data);
}
